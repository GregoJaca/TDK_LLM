...
from src.config import Experiment, Analysis, Default, Prompts
...
for rrr in Experiment.RADII:
    for ttt in range(len(Experiment.TEMPS)):
        for prompt_idx in range(len(Prompts.prompts)):
            ...
            hypervolumes = None
            axis_lengths = None
            if Analysis.PLOT_HYPER_AND_AXIS:
            ...
            if Analysis.PLOT_HYPER_AND_AXIS and hypervolumes is not None and axis_lengths is not None:
            ...
            if Analysis.RUN_LOCAL_DIMENSIONALITY:
        ...
        # --- Rank Eigenvectors of pairs of trajectories ---
            ...
            if Analysis.RUN_RANK_EIGENVECTORS:
                for i, j in Analysis.PAIRS_TO_PLOT:
                    ...
                    try:
                        ...
                        if Analysis.SAVE_PLOTS:
                            outpath = os.path.join(PLOTS_DIR, f"rank_eigen_pca_{i}_{j}.png")
                            plt.savefig(outpath)
                            print(f"Saved full-rank plot: {outpath}")
                            plt.clf()
                        else:
                            plt.show()
                        ...
                        if positions.size > 0:
                            ...
                            plt.tight_layout()
                            if Analysis.SAVE_PLOTS:
                                outpath2 = os.path.join(PLOTS_DIR, f"rank_eigen_pca_{i}_{j}_sliding.png")
                                plt.savefig(outpath2)
                                print(f"Saved sliding-rank plot: {outpath2}")
                                plt.clf()
                            else:
                                plt.show()

                    except IndexError as e:
                        print(f"Skipping pair {i},{j} — cannot index trajectories: {e}")
                        continue
